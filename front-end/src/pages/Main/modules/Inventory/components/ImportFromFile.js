...
export default function ImportFromFile() {
  ...
  const handleSelectFile = async (e) => {
    ...
    try {
      ...
      show(ToastTypes.SUCCESS, "Successfully imported to " + res.data.importedProductNumber + " products!");
      setInsertDone(true);
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    <Box>
      <BackButton />
      <Box className={'mt-3 bg-light d-flex justify-content-center align-items-center flex-column'}
        style={{
          height: '30vh'
        }}
      >
        ...
        <Typography variant={'subtitle2'}>
          Support files are xls, xlsx, json.
        </Typography>
        <Typography variant={'subtitle2'}>
          <a target={'_blank'} href={process.env.REACT_APP_SERVER_HOST + `/my-product-templates.xlsx`}>xlsx template</a>
        </Typography>
        <Typography variant={'subtitle2'}>
        ...
        {files.map(file => {
          return (
            <ListItem key={file.lastModified}>
              <Box className={'p-3 bg-light w-100'}>
                <Typography
                  className={'fw-bold'}
                  variant={'subtitle2'}>
                  {file.name}
